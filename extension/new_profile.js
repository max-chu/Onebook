...
function init(){
    ...
    if (document.location.href.startsWith("https://www.linkedin.com/in/")){
        for(let con of connections){
            if (con.socials.linkedIn && document.location.href === con.socials.linkedIn){
                displayContact(con);
                return;
            }
        }
...
function promptNewContact(){
    ...
    if(potentialCons.length !== 0){
        ...
        let p = document.createElement("p");
        let node = document.createTextNode("This profile may belong to one of your existing users. Would you like to add this profile to one of these accounts?");
        p.appendChild(node);
        noteNode.appendChild(p);
        ...
        for(let con of potentialCons){
            ...
            button.addEventListener("click", function (){
                connections.find((contact) => contact.name === con.name).socials.linkedIn = document.location.href;
        ...
        let p = document.createElement("p");
        let node = document.createTextNode("We couldn't find any existing connections that might own this account.");
        p.appendChild(node);
        noteNode.appendChild(p);
    }
